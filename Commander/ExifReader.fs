...
let toDate (datestr: string) = 
    DateTime.ParseExact(datestr.TrimEnd( [| char 0 |]), "yyyy:MM:dd HH:mm:ss", CultureInfo.InvariantCulture).ToUniversalTime()

let getExifDate file =     
    use reader = getExif file
    if reader <> null then 
        Some (reader.GetTagValue 0x9003us :?> string |> toDate)
    else 
        None
